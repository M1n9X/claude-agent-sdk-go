...
// TestConfigureMCPServers ensures SDK MCP server definitions from options are registered.
func TestConfigureMCPServers(t *testing.T) {
	ctx := context.Background()
	transport := newMockTransport()
	...
	serverConfig := types.CreateToolServer("local", "1.0.0", []types.McpTool{tool})
	options := types.NewClaudeAgentOptions().
		WithMcpServers(map[string]interface{}{
			"local": serverConfig,
		})

	query := NewQuery(ctx, transport, options, logger, true)
	if err := query.ConfigureMCPServers(options); err != nil {
		t.Fatalf("ConfigureMCPServers failed: %v", err)
	}
	...
	mcpResponse, ok := result["mcp_response"].(map[string]interface{})
	if !ok {
		t.Fatalf("expected mcp_response, got %v", result)
	}

	if mcpResponse["result"] == nil {
		t.Fatalf("expected successful result, got %v", mcpResponse)
	}
}

// TestConfigureMCPServersErrors validates error handling for invalid MCP server configs.
func TestConfigureMCPServersErrors(t *testing.T) {
	ctx := context.Background()
	transport := newMockTransport()
	logger := log.NewLogger(false)
	query := NewQuery(ctx, transport, nil, logger, true)

	options := types.NewClaudeAgentOptions().
		WithMcpServers("/tmp/mcp.json")

	if err := query.ConfigureMCPServers(options); err != nil {
		t.Fatalf("expected nil error for string MCP servers config, got %v", err)
	}

	badConfig := &types.ToolServerConfig{Type: "sdk", Name: "broken"}
	options.McpServers = map[string]interface{}{"broken": badConfig}
	if err := query.ConfigureMCPServers(options); err == nil {
		t.Fatal("expected error for missing SDK MCP server instance")
	}
}
