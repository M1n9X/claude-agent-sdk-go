	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				...
				if !types.IsMessageParseError(err) && !types.IsJSONDecodeError(err) && !types.IsCLIJSONDecodeError(err) {
					t.Errorf("expected MessageParseError, JSONDecodeError, or CLIJSONDecodeError, got %T", err)
				}
			}
		})
	}
