...
func (c *Client) Connect(ctx context.Context) error {
	...
	if err := c.query.ConfigureMCPServers(c.options); err != nil {
		c.logger.Error("Failed to configure MCP servers: %v", err)
		_ = c.transport.Close(ctx)
		return err
	}
...
// Interrupt sends an interrupt request to Claude, stopping the current execution.
//
// This allows stopping a long-running task initiated by Claude (e.g., a long tool use or response).
// This requires an active connection and will have no effect if not connected.
//
// Parameters:
//   - ctx: Context for cancellation
//
...
//	}
//
//	// Later, send an interrupt
//	go func() {
//	    time.Sleep(5 * time.Second) // Interrupt after 5 seconds
...
func (c *Client) Interrupt(ctx context.Context) error {
	...
	// Build interrupt message with a generated request ID
	...
	if err != nil {
		return types.NewControlProtocolErrorWithCause("failed to marshal interrupt message", err)
	}
